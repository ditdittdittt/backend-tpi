...
func (t *transactionItemRepository) GetReport(tpiID int, districtID int, from string, to string) ([]map[string]interface{}, error) {
	...
	query := `SELECT ti.id, t.code, f.name AS fisher_name, b.name AS buyer_name, ft.name AS fish_name, ft.code AS fish_code, ci.weight, ci.weight_unit, a.price 
	...
	query = query + ` AND t.created_at BETWEEN "%s" AND "%s" AND ci.caught_status_id = 3`
